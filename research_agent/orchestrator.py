...
from research_agent.models import ResearchPlan, ToolCall, ToolResult, Source, ResearchReport
    ...
    def run_research(self, question: str) -> ResearchReport:
        ...
        try:
            ...
            while execution_steps_count < settings.MAX_STEPS:
                ...
                if is_duplicate:
                    self.logger.log_warning(f"Duplicate tool call detected at Step {execution_steps_count}. Forcing early synthesis to avoid infinite loop.")
                    break
                ...
                if tool_call.tool_name == "search_web":
                    ...
                    # If scraping succeeded, add to verified sources pool
                    if tool_result.success and url not in scraped_urls:
                        scraped_urls.add(url)
                        # Extract title and snippet for sources list
    ...
    def _decide_next_step(self, question: str, plan: ResearchPlan, history: List[Dict[str, Any]], step_num: int) -> ToolCall:
        ...
        for step in history:
            ...
            history_str += (
                ...
                f"- Tool Called: {prev_result.get('tool_name')}\n"
    ...
    def _synthesize_report(self, question: str, plan: ResearchPlan, history: List[Dict[str, Any]], scraped_sources: List[Dict[str, Any]]) -> ResearchReport:
        ...
        for step in history:
            ...
            if prev_result.get("tool_name") == "fetch_webpage" and prev_result.get("success"):
                ...
                facts_str += f"Webpage Data from {prev_result.get('url')}:\n{content}\n\n"
